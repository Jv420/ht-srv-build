...
local function sanitizeRgb(value)
    local output = { 255, 255, 255 }
    if type(value) ~= 'table' then return output end
    for i = 1, 3 do
        output[i] = math.max(0, math.min(255, math.floor(tonumber(value[i]) or 255)))
    end
    return output
end

local function sanitizeNeonEnabled(value)
    local output = { false, false, false, false }
    if type(value) ~= 'table' then return output end
    for i = 1, 4 do output[i] = value[i] == true end
    return output
end
...
local function sanitizeExtras(value)
    ...
    for key, enabled in pairs(value) do
        ...
        if extraId and extraId >= 0 and extraId <= 20 then
            output[tostring(math.floor(extraId))] = enabled == true or enabled == 1
        end
    end
    return output
end
...
local function sanitizeVehicleProps(props, modelHash, newPlate)
    ...
    local safe = {
        ...
        plate = newPlate,
        plateIndex = math.max(0, math.min(5, tonumber(Config.plateIndex) or tonumber(props.plateIndex) or 1)),
        ...
        extras = sanitizeExtras(props.extras),
        neonEnabled = sanitizeNeonEnabled(props.neonEnabled),
        neonColor = sanitizeRgb(props.neonColor),
        tyreSmokeColor = sanitizeRgb(props.tyreSmokeColor)
    }
...
RegisterNetEvent('ht_chopshop:server:tracker', function(netId, action)
    ...
    if action == 'add' then
        ...
        local affected = MySQL.update.await(
            'UPDATE `ht_vehicle_registry` SET `tracker_enabled` = 1, `tracker_code` = ?, `updated_at` = NOW() WHERE `plate` = ? AND `tracker_enabled` = 0',
            { trackerCode, plate }
        )
        if not affected or affected < 1 then
            ox_inventory:AddItem(source, itemName, 1)
            return notify(source, 'Tracker kon niet worden opgeslagen; het onderdeel is teruggegeven.', 'error')
        end
    ...
    if action == 'remove' then
        ...
        local affected = MySQL.update.await(
            'UPDATE `ht_vehicle_registry` SET `tracker_enabled` = 0, `tracker_code` = NULL, `updated_at` = NOW() WHERE `plate` = ? AND `tracker_enabled` = 1',
            { plate }
        )
        if not affected or affected < 1 then
            ox_inventory:AddItem(source, itemName, 1)
            return notify(source, 'Tracker kon niet worden verwijderd; het gereedschap is teruggegeven.', 'error')
        end
