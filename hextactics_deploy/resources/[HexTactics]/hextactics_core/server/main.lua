...
local function parseIdentifier(identifier)
    ...
    if characterId then identifier = identifier:gsub('^char%d+:', '', 1) end
    local identifierType, value = identifier:match('^([^:]+):(.+)$')
    if identifierType == 'license' or identifierType == 'license2' then identifier = value end
...
local function identifiersMatch(left, right)
    ...
    if leftBase == '' or rightBase == '' or leftBase ~= rightBase then return false end
    if leftCharacter and rightCharacter then return leftCharacter == rightCharacter end
    return true
end
...
local function getPlayerIdentity(source)
    ...
    local function add(identifier)
        ...
        if identifierType and not typed[identifierType] then typed[identifierType] = identifier end
    end
    local activeIdentifier = getActiveIdentifier(source)
    add(activeIdentifier)
    for _, identifier in ipairs(GetPlayerIdentifiers(source)) do add(identifier) end
