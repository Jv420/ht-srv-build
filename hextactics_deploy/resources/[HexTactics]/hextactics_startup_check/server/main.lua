...
local function runCheck(source)
    ...
    for _, resource in ipairs(optionalResources) do
        local state = GetResourceState(resource)
    ...
    for _, resource in ipairs(conflictingResources) do
        local state = GetResourceState(resource)
    ...
    if #missing == 0 and #conflicts == 0 then
        print('^2[HexTactics] Verplichte basiscontrole geslaagd.^7')
    else
        if #missing > 0 then
            print(('^3[HexTactics] Niet gestart: %s^7'):format(table.concat(missing, ', ')))
        end
        if #conflicts > 0 then
            print(('^1[HexTactics] Schakel deze conflicten uit: %s^7'):format(table.concat(conflicts, ', ')))
        end
    end

    if #optionalMissing > 0 then
        print('^3[HexTactics] LC Fuel is niet automatisch meegeleverd. Upload jouw legale lc_fuel-resource handmatig.^7')
    end
    ...
    if source and source > 0 then
        TriggerClientEvent('chat:addMessage', source, {
            args = {
                'HexTactics',
                (#missing == 0 and #conflicts == 0)
                    and 'Basiscontrole geslaagd. Bekijk de serverconsole voor LC Fuel en details.'
                    or 'Er zijn ontbrekende resources of conflicten. Bekijk de serverconsole.'
            }
        })
    end
end
