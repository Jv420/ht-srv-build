...
async function post(action) {
  const response = await fetch(`https://${GetParentResourceName()}/action`, {method:'POST',headers:{'Content-Type':'application/json; charset=UTF-8'},body:JSON.stringify({action})});
...
window.addEventListener('message', (event) => {
  ...
  } else if (data.type === 'close') body.classList.remove('visible');
});
